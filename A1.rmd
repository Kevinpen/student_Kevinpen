...
```{r}
hgnc_checker <- read.csv("./hgnc-symbol-check.csv", header = FALSE)
...
Looking at the unmatched rows in "hgnc-symbol-check.csv", I realized all unmatched symbols have "." in it. For example symbols start with "RP". I guess it's denotes repeats, not really gene name. Lecture slide also mentioned this pattern.
...
No normalization actually needed, but for practice doing normalization one more time to see if there are any effect.
...
The plot is slightly different from the plot of distribution before normalization, but basically the same. The two plots have roughly same distribution range.
...
Since the normalization not really needed, I keep the data set just before normalization as final result.

Save the result:
```{r}
result<-as.data.frame(car_exp_filtered)
...
__Conclusion__: The test group samples have smaller distance within group than between test and control group, showing test sample data are more similar within each other with than with data of control group.
